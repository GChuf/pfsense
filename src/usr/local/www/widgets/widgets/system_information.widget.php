...
?>
		...
		<tr>
			...
			<td>
				...
				<span><?=$swapusage?>% (<?= sprintf("%.0f", `/usr/sbin/swapinfo -m | /usr/bin/tail -1 | /usr/bin/awk '{ print $3;}'`) ?>/<?= sprintf("%.0f", `/usr/sbin/swapinfo -m | /usr/bin/tail -1 | /usr/bin/awk '{ print $2;}'`) ?> MiB)</span>
			</td>
		</tr>
		<?php endif; ?>

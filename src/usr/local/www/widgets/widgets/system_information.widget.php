...
//cache
$bootTime = g_get('cache_boot_time');
$cpuMaxFreq = g_get('cache_cpu_max_speed');
$maxMem = intval(g_get('cache_hw_real_mem') / 1048576);
...
?>
                <tr>
                        ...
                        <td><?=htmlspecialchars(get_single_sysctl("hw.model"))?>
								<div>
                                <span style="display: inline;"><span>Current: </span><span id="cpufreq"></span><span> MHz, Max: <?=$cpuMaxFreq?> Mhz</span></span>
								</div>
                <?php
...
?>
                <tr>
                        ...
                        <td>
                                ...
                                <span style="display: inline;"><span id="memusagemeter"></span><span>% of <?=$maxMem?> MiB</span></span>
                        </td>
                </tr>
<?php
        endif;
...
function updateUptime(x) {
        if ($('#uptime')) {
				...
				let days = Math.floor(unixTime / (24 * 3600));
				unixTime %= (24 * 3600);
				let hours = Math.floor(unixTime / 3600);
				unixTime %= 3600;
				...
				if (days > 0) {
					output += (days > 1 ? `${days} days, ` : '1 day, ');
				}

				output += (hours > 1 ? `${hours} hours, ` : '1 hour, ');


				output += (minutes > 1 ? `${minutes} minutes, ` : '1 minute, ');
...
function updateCpuFreq(x) {
        if ($('#cpufreq')) {
		//	var result = "Current: " + x + " MHz, Max: " + cpuMaxFreq + " Mhz";
